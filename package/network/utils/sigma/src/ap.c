...
void btm_add_neighbor(struct sigma_dut *dut, char *cmd_buf){
    ...
    if(add_default){
        if(parse_mac_address(dut, g_wlan_mac, m) < 0)
            sprintf(buf, " neighbor=10:16:88:c1:0b:c4,23,115,36,0,0603000000030100");
        else
				     ...
				     struct sigma_cmd *cmd, const char *ifname)
{
    ...
    if(BTM_DisAssoc_Imnt) {
        ...
        btm_add_int("disassoc_timer", 1000, buf);
    } else {
        btm_add_int("disassoc_imminent", 0, buf);
        //hostapd里面不管disassoc_imminent字段，只要disassoc_timer里有值就经过这个值ms断开，0就不断（不发disassoc）
        btm_add_int("disassoc_timer", 10, buf);
    }

    btm_add_int("valid_int", 115, buf);
